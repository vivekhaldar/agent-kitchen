...
function createChatEnv() {
  const html = `<!DOCTYPE html><html><body>
    <div class="chat-panel" id="chat-panel">
      ...
      <div class="chat-empty" id="chat-empty"></div>
    </div>
  </body></html>`;
...
describe("local command detection", () => {
  let win, api;
  ...
  it("parses command-name XML into command type", () => {
    const result = api.parseLocalCommand(
  ...
  it("parses local-command-stdout XML into stdout type", () => {
    const result = api.parseLocalCommand(
      "<local-command-stdout>Set model to claude-opus-4-6[1m]</local-command-stdout>"
    );
    assert.equal(result.type, "stdout");
    assert.equal(result.text, "Set model to claude-opus-4-6[1m]");
  });

  it("parses local-command-caveat as caveat type", () => {
    const result = api.parseLocalCommand(
      '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>'
    );
    assert.equal(result.type, "caveat");
  });

  it("returns null for regular user text", () => {
    assert.equal(api.parseLocalCommand("Hello, how are you?"), null);
  });

  it("renders command as compact pill, not user bubble", () => {
    const tab = makeTab(win);
    api.appendLocalCommand(tab, { type: "command", name: "/model" });
    const el = tab.container.querySelector(".chat-local-command");
    assert.ok(el, "should render a .chat-local-command element");
    assert.ok(el.textContent.includes("/model"));
    assert.equal(tab.container.querySelectorAll(".chat-bubble.user").length, 0,
      "should NOT render a user bubble");
  });

  it("skips rendering for caveat type", () => {
    const tab = makeTab(win);
    api.appendLocalCommand(tab, { type: "caveat" });
    assert.equal(tab.container.children.length, 0);
  });

  it("user_message_chunk routes local commands to compact renderer", () => {
    const tab = makeTab(win);
    api.handleUpdate(tab, {
      sessionUpdate: "user_message_chunk",
      content: { text: '<command-name>/exit</command-name>\n            <command-message>exit</command-message>\n            <command-args></command-args>' },
    });
    assert.ok(tab.container.querySelector(".chat-local-command"),
      "should render compact local command");
    assert.equal(tab.container.querySelectorAll(".chat-bubble.user").length, 0,
      "should NOT render user bubble for local commands");
  });
});
